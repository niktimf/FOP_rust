...
fn main() {
    ...
    /// Чистая функция
    fn add_one_map(vec: &[i32]) -> Vec<i32> {
    ...
    /// В большинстве случаев, вы легко определите побочная(в плане изменения состояния переменной) ваша функция или нет,
    /// потому что будет использоваться мутабельная ссылка (&mut) и сама переменная будет мутабельной (let mut).
    ...
    fn add_one_map_print(vec: &[i32]) -> Vec<i32> {
        vec.iter().map(|x| {
            println!("x = {}", x);
            x + 1
        }).collect()
